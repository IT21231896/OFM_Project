...
export default function Product() {
  ...
  const filterByPriceRange = () => {
    if (priceRange === 'All Price') {
      ...
      return products.filter((product) => product.price >= 100 && product.price <= 3000);
        ...
        <div className="col-md-8">
          ...
          <h6 className="product-view-name mb-4">Viewing {filteredProducts().length} of {products.length} products</h6>
            ...
            {filteredProducts().map(product => (
              ...
              <Link to={`/product/${product._id}`}><button className='text-white product-btn'>Add to Cart</button></Link>
             </div>
            ))}
